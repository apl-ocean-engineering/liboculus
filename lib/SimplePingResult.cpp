...
MessageBuffer::MessageBuffer() : _buf(sizeof(OculusMessageHeader), 0) { ; }

MessageBuffer::MessageBuffer(const char *data, size_t len) : _buf(len) {
  memcpy(_buf.data(), data, len);
}

MessageBuffer::MessageBuffer(const std::vector<char> &vec) : _buf(vec) { ; }

MessageBuffer::~MessageBuffer() { ; }

char *MessageBuffer::ptr() { return _buf.data(); }
...
unsigned int MessageBuffer::size() const { return _buf.size(); }
